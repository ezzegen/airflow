        ...
        for k in range(1, 11):
            ...
            for col in df_challenge.columns:
                ...
                for i in range(len(df)):
                    ...
                    X_pred = df_history.iloc[[i], -k:]  # здесь оставляем X_pred как DataFrame
                    y_pred = model.predict(X_pred)  # теперь это DataFrame
                    y_pred_list.append(y_pred[0])  # берем первое значение предсказания
